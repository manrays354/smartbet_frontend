...
import { 
  Trophy, Lock, Crown, Zap, ChevronRight, 
  Phone, X, TrendingUp
} from 'lucide-react';
...
// RENAME TO 'App' AND EXPORT AS DEFAULT
export default function App() {
  const [view, setView] = useState<'today' | 'history'>('today');
  // Load phone from localStorage so user stays logged in
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(`${API_BASE_URL}`, {
        params: { phone: phoneNumber } // Sends ?phone=...
      });
      return res.data;
    },
    staleTime: 1000 * 60 * 5,
  ...
  const handlePayment = async (e: FormEvent) => {
    ...
    try {
      ...
      alert("M-Pesa STK Push Sent! Enter PIN. Refresh in 10s.");
      setIsModalOpen(false);
      setTimeout(() => refetch(), 15000);
    } catch (err) { 
      alert("Payment initiation failed. Please check your phone number format (254...)."); 
    } finally { 
      setIsPaying(false); 
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfd] font-sans text-slate-900 flex flex-col">
          ...
          <div key={date} className="mb-14">
            ...
            <div className="space-y-6">
              {segments.pro.map((game: any) => {
                ...
                return (
                  <div key={game.id} onClick={() => isLocked && setIsModalOpen(true)} className={`rounded-[2.2rem] border p-6 transition-all cursor-pointer ${isLocked ? 'bg-slate-900 text-white shadow-xl' : 'bg-white border-slate-100'}`}>
                    <div className="flex justify-between items-center mb-4">
                      <span className="text-[9px] font-black px-2.5 py-1 rounded-lg uppercase bg-amber-500 text-slate-900">VIP</span>
                      {!isLocked ? <span className="text-amber-500 font-black text-sm">{game.odds}</span> : <span className="text-slate-700 font-black text-[9px] uppercase"><Lock size={10}/> Hidden</span>}
                    </div>
                    <h4 className="text-lg font-bold mb-5 italic">{game.title}</h4>
                    <div className={`p-5 rounded-3xl border ${isLocked ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-100'}`}>
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Expert Verdict</p>
                      <p className={`font-black text-lg ${isLocked ? 'text-amber-500 blur-md' : 'text-slate-700 italic'}`}>{isLocked ? "LOCKED" : game.predicted_outcome}</p>
                    </div>
                  </div>
                );
              })}
      ...
      {/* PAYMENT MODAL (Maintains original CSS) */}
      <AnimatePresence>
        {isModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl">
              <div className="flex justify-between items-center mb-8">
                <div className="bg-amber-50 p-3 rounded-2xl"><Zap className="text-amber-500" size={24} fill="currentColor" /></div>
              ...
              <p className="text-slate-500 text-sm mb-8">Get 24hr access via M-Pesa STK Push.</p>
              <form onSubmit={handlePayment} className="space-y-4">
                <div className="relative">
                  <Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                  <input type="tel" required placeholder="2547XXXXXXXX" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} className="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-2xl font-bold focus:ring-2 ring-amber-500 outline-none" />
                </div>
                <button disabled={isPaying} className="w-full bg-slate-900 text-white font-black py-4 rounded-2xl shadow-lg disabled:opacity-50">{isPaying ? "SENDING PUSH..." : "PAY KES 200 NOW"}</button>
              </form>
            </motion.div>
          </motion.div>
        )}
